...
func (w *Writer) Write(eventBytes []byte) (int, error) {
	...
	defer func() {
		closeErr := fp.Close()
		if closeErr != nil {
			logger.ERROR.Printf("failed to close %s after write: %v", filename, closeErr)
		}
	}()
...
func escapeLine(line []byte) []byte {
	return []byte(escapeReplacer.Replace(string(line)))
}

func UnescapeLine(line []byte) []byte {
	return []byte(unescapeReplacer.Replace(string(line)))
}
